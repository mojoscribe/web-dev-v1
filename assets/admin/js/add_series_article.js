...
$(document).ready(function(){
	...
	function uploadFile() {
		...
		xhr.open("POST", base_url + "/series/upload");  
		xhr.send(data);
	...
	function saveArticle(status){
		...
		if(articleId == 0) {
			var data = {title: title, content: content, status: status, color: selectedColor, excerpt: excerpt, category:category, seriesTitle:seriesTitle};
		} else {
			var data = {title: title, content: content, status: status, id: articleId, color: selectedColor, excerpt: excerpt, category:category, seriesTitle:seriesTitle};
		}
		
		$.ajax({
			url: base_url + '/article/saveArticle',
			type: 'POST',
			data: data,
